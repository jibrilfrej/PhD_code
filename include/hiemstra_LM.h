...
//Same as before but with a smoothing that takes into account the collection frequency
double Hiemstra_indexed_language_model(const std::vector<int> &query , const std::vector<int> &document , const std::unordered_map <int,int>  &cf , const int collection_size , const double lambda){
...
//Same as before but with a smoothing that takes into account the collection frequency
std::unordered_map <int,double> Hiemstra_indexed_language_model(const std::vector<int> &query , const std::unordered_map < int , std::vector<int> > &collection , const std::unordered_map <int,int>  &cf , const int collection_size , const double lambda){
	...
	while(iterator != collection.end()){
		...
		proba = Hiemstra_indexed_language_model(query , iterator->second , cf , collection_size , lambda);
		if(proba!=0){list_doc[iterator->first]=proba;}
...
//Same as before but with all the queries and sort documents by their score
std::vector< std::vector< std::pair<int,double> > > Hiemstra_indexed_language_model(const std::unordered_map< int , std::vector<int> > &queries , const std::unordered_map < int , std::vector<int> > &collection , const std::unordered_map <int,int>  &cf , const int collection_size , const int k , const double lambda){
	...
	while(iterator != queries.end()){
		...
		if(iterator->second.size()!=0){
			...
			list_docs[iterator->first] = kfirst_docs( Hiemstra_indexed_language_model(iterator->second , collection , cf , collection_size , lambda) , k );
