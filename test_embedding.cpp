...
int main(int argc, char** argv) {
	...
	embedding.load_Word2VecBinFormat("../data/embeddings/GoogleNews-vectors-negative300/GoogleNews-vectors-negative300.bin");
	embedding.display_attributes();
	float* temp;
	clock_t begin = clock();
  ...
  double elapsed_secs = double(end - begin) / CLOCKS_PER_SEC;
	...
	cout<<"Cosine similarity between test and test : "<<embedding.cosine("test","test") <<endl;

	cout<<"Cosine similarity between test and tests : "<<embedding.cosine("test","tests") <<endl;

	cout<<"Cosine similarity between test and dfghtgh : "<<embedding.cosine("test","dfghtgh") <<endl;
